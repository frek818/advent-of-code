...
class Rucksack:
    def __init__(self, content):
        assert len(content) % 2 == 0, f"Compartment aren't equal size {len(content)}"
    ...
    def common_item_between_compartments(self):
        common_item = set(self.compartment_1).intersection(set(self.compartment_2))
        assert (
            len(common_item) == 1
        ), f"Multiple common items between compartments: {common_items}"
...
def common_item_in_group(group) -> List[FrozenSet[str]]:
    ...
    assert len(common_badge) == 1, f"Too many common item in group: {common_badge}"
